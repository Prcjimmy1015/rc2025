...
**测试命令**：
```bash
sudo python3 arm_task/task_planner.py --stage 1    # 阶段1: 抓取平台装货
sudo python3 arm_task/task_planner.py --stage 2    # 阶段2: 中转平台卸货+装货
sudo python3 arm_task/task_planner.py --stage 3    # 阶段3: 检测点警示标志识别
sudo python3 arm_task/task_planner.py --stage 4 --target 1  # 阶段4: 一号放置平台卸货
```
...
```bash
# 逐阶段测试（需 sudo）
sudo python3 arm_task/task_planner.py --stage 1    # 期望输出 MARKER_RESULT=N
sudo python3 arm_task/task_planner.py --stage 2    # 期望 exit 0
sudo python3 arm_task/task_planner.py --stage 3    # 期望输出 WARNING_ID=N
sudo python3 arm_task/task_planner.py --stage 4 --target 1
sudo python3 arm_task/task_planner.py --stage 4 --target 2
```

| 阶段 | 通过标准 |
|------|---------|
| 阶段1 | 成功抓取起始物资 + 输出 MARKER_RESULT |
| 阶段2 | 成功卸载起始物资 + 抓取场地物资 |
| 阶段3 | 成功识别警示标志 + 回到抓取行走姿态 |
| 阶段4 | 成功卸载场地物资到指定平台 |
...
- [ ] `POSE_NAVIGATION`: 导航（空载行走）关节角度 → 3个文件自动加载
...
- [ ] `_PIXEL_POINTS` / `_WORLD_POINTS`: 像素→世界标定点 → 取消注释计算 `AFFINE_MATRIX`
