...
保存为 JSON 文件供 vision_utils.py 加载使用。

用法：
  # 步骤1: 采集标定点（交互式）
  python3 calibrate_affine.py --collect

  # 步骤2: 从采集的数据计算矩阵 + 保存
  python3 calibrate_affine.py --compute

  # 步骤3: 可视化验证
  python3 calibrate_affine.py --verify
...
# 数据文件路径
POINTS_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "calib_points.json")
        ...
        while True:
            ...
            while True:
                ...
                if key == ord('q'):
                    ...
                    click_pt.clear()
                    break
                elif click_pt and key == 13:  # Enter 确认
                    break
...
def compute_matrix():
    ...
    if not os.path.exists(POINTS_FILE):
        ...
        print("请先运行: python3 calibrate_affine.py --collect")
        return
    ...
    # 注意: world 顺序为 (Z, X) 以匹配 OpenCV 期望
    ...
    for i, p in enumerate(points):
        ...
        print(f"  点{i+1}: 像素({p['pixel_x']},{p['pixel_y']}) → "
              f"计算世界(Z={wz_calc:.1f}, X={wx_calc:.1f}), "
              f"误差={err:.1f}mm")
...
def verify_matrix():
    ...
    if not os.path.exists(MATRIX_FILE):
        ...
        print("请先运行: python3 calibrate_affine.py --compute")
        return
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
完整流程:
  1. python3 calibrate_affine.py --collect    # 采集3+组点对
  2. python3 calibrate_affine.py --compute    # 计算矩阵 + 保存
  3. python3 calibrate_affine.py --verify     # 验证矩阵
        """,
    )
    parser.add_argument(
    ...
    if args.collect:
        ...
        print("或查看帮助: python3 calibrate_affine.py -h")