...
static void task0_loop(go2::SportClient &sc, const Mat &u, double ly)
{
    ...
    int rh = 40, ys = b.rows - rh;
    if (ys < 0)
        ys = 0;
    double e = 0;
    int c = 0, rw = n.cols;
    vector<int> cc(rw, 0);
    for (int r = ys; r < b.rows; ++r)
    {
    ...
    {
        if (ok)
            cout << "[T0] LINE err=" << e << " c=" << c << " pk=" << pk << endl;
        else
            cout << "[T0] NO  LINE c=" << c << " pk=" << pk << endl;
    }
    ...
    {
        static double I = 0, Le = 0;
        I += e;
        I = max(-50., min(50., I));
        double s = -(0.12 * e + 0.002 * I + 0.01 * (e - Le));
        Le = e;
        s = max(-0.8, min(0.8, s));
        if (abs(lc) > 0.01)
            s = lc;
               ...
               const unitree_go::msg::dds_::SportModeState_ &state, int fcount)
{
    ...
    static int dbg = 0;
    if (dbg++ % 15 == 0)
        printf("[case0] st=%d lx=%.2f px=%.2f py=%.2f\n", st, lx, px, py);
    if (g_case0_skip_init && st < 3)
    {
        st = 3;
        cout << "=== T0 ACTIVE ===" << endl;
    }
