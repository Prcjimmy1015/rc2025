...
if(n_st>=30){settled=true;cout<<"[V14] Settled, go.\n"<<endl;}return;}
...
if(cnt%15==0){if(ok)printf("[V14] LINE err=%.0f ci=%d pk=%d\n",e,ci,pk);else printf("[V14] NO LINE ci=%d pk=%d\n",ci,pk);}
...
if(ok&&abs(e)<400&&ci>100){
    ...
    else{double tg=e/1280.0*60.0*M_PI/180.0;double s=-tg*4.5;s=max(-1.0,min(1.0,s));if(abs(lc)>0.01)s=lc;s=max(-1.0,min(1.0,s));double vx=(abs(e)>300)?0.08:0.15;sc.Move(vx,0,s);if(cnt%15==0)printf("[V14] servo s=%.2f vx=%.2f\n",s,vx);}
}else if(ok){
...
if(g_case0_skip_init){t0(sc,undist);if(g_enable_gui){double fps=fc/chrono::duration<double>(chrono::steady_clock::now()-t0t).count();putText(undist,format("V14 FPS %.1f",fps),{10,30},FONT_HERSHEY_SIMPLEX,1,{0,255,0},2);imshow("Go2",undist);if(waitKey(1)==27)break;}continue;}
...
if(g_enable_gui){double fps=fc/chrono::duration<double>(chrono::steady_clock::now()-t0t).count();putText(undist,format("V14 FPS %.1f",fps),{10,30},FONT_HERSHEY_SIMPLEX,1,{0,255,0},2);imshow("Go2",undist);if(waitKey(1)==27)break;}}
