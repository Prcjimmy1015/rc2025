...
static void t0(go2::SportClient &sc, Mat &f){
    ...
    // ★ 前15帧只低头不动，稳定后再巡线
    static bool settled=false; static int scnt=0;
    if(!settled){scnt++;sc.StaticWalk();sc.Euler(0,0.4,0);sc.Move(0,0,0);if(scnt>=15){settled=true;cout<<"[V4] Settled, go.\n"<<endl;}return;}
    ...
    int rh=100,ys=b.rows-rh;if(ys<0)ys=0;double e=0;int c=0,rw=n.cols;vector<int>cc(rw,0);
    ...
    if(cnt%15==0){if(ok)printf("[V4] LINE err=%.0f c=%d pk=%d\n",e,c,pk);else printf("[V4] NO LINE c=%d pk=%d\n",c,pk);}
    double ly=0;{double _,dy;transformLocal(px,py,yaw,_,ly,dy);}double lc=(ly>0.35)?-0.3:(ly<-0.35)?0.3:0;
    if(ok&&abs(e)<400&&c>100){double tg=e/1280.0*60.0*M_PI/180.0;double s=tg*3.0;s=max(-0.8,min(0.8,s));if(abs(lc)>0.01)s=lc;s=max(-0.8,min(0.8,s));double vx=(abs(e)>300)?0.12:0.25;sc.StaticWalk();sc.Euler(0,0.4,0);sc.Move(vx,0,s);if(cnt%15==0)printf("[V4] servo s=%.2f vx=%.2f\n",s,vx);}
    else if(ok){double tg=e/1280.0*60.0*M_PI/180.0;if(abs(e)>=400){double s=max(-0.8,min(0.8,tg*3.0));sc.StaticWalk();sc.Euler(0,0.4,0);sc.Move(0,0,s);if(cnt%15==0)printf("[V4] STOP+TURN\n");}else{double s=max(-0.8,min(0.8,tg*3.0));if(abs(lc)>0.01)s=lc;s=max(-0.8,min(0.8,s));sc.StaticWalk();sc.Euler(0,0.4,0);sc.Move(0.15,0,s);}}
...
int main(int ac,char**av){
    ...
    while(!g_exit){if(!cap.read(frame)||frame.empty())break;fc++;undistort(frame,undist,K,D);if(g_force_task>=0)Flag_Task=g_force_task;
        if(g_case0_skip_init){t0(sc,undist);if(g_enable_gui){double fps=fc/chrono::duration<double>(chrono::steady_clock::now()-t0t).count();putText(undist,format("V4 FPS %.1f",fps),{10,30},FONT_HERSHEY_SIMPLEX,1,{0,255,0},2);imshow("Go2",undist);if(waitKey(1)==27)break;}continue;}
