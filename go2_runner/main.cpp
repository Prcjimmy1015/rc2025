...
if(cnt%15==0){if(ok)printf("[V3] LINE err=%.0f px c=%d pk=%d\n",e,c,pk);else printf("[V3] NO LINE c=%d pk=%d\n",c,pk);}
double ly=0;{double _,dy;transformLocal(px,py,yaw,_,ly,dy);}double lc=(ly>0.35)?-0.3:(ly<-0.35)?0.3:0;
if(ok&&abs(e)<400&&c>100){double t=e/1280.0*60.0*M_PI/180.0;double s=t*3.0;s=max(-0.8,min(0.8,s));if(abs(lc)>0.01)s=lc;s=max(-0.8,min(0.8,s));double vx=(abs(e)>300)?0.12:0.25;sc.StaticWalk();sc.Euler(0,0.4,0);sc.Move(vx,0,s);if(cnt%15==0)printf("[V3] servo steer=%.2f vx=%.2f t=%.1fdeg\n",s,vx,t*180/M_PI);}
else if(ok){double t=e/1280.0*60.0*M_PI/180.0;if(abs(e)>=400){double s=max(-0.8,min(0.8,t*3.0));sc.StaticWalk();sc.Euler(0,0.4,0);sc.Move(0,0,s);if(cnt%15==0)printf("[V3] STOP+TURN steer=%.2f t=%.1fdeg\n",s,t*180/M_PI);}else{double s=max(-0.8,min(0.8,t*3.0));if(abs(lc)>0.01)s=lc;s=max(-0.8,min(0.8,s));sc.StaticWalk();sc.Euler(0,0.4,0);sc.Move(0.15,0,s);}}
...
if(g_case0_skip_init){t0(sc,undist);if(g_enable_gui){double fps=fc/chrono::duration<double>(chrono::steady_clock::now()-t0t).count();putText(undist,format("V3 FPS %.1f",fps),{10,30},FONT_HERSHEY_SIMPLEX,1,{0,255,0},2);imshow("Go2",undist);if(waitKey(1)==27)break;}continue;}
