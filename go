                ...
                double ly)
{
    ...
    Ptr<cv::aruco::DetectorParameters> p = cv::aruco::DetectorParameters::create();
    Ptr<cv::aruco::Dictionary> d = cv::aruco::getPredefinedDictionary(cv::aruco::DICT_4X4_50);
    vector<int> ids;
    vector<vector<Point2f>> corners;
    cv::aruco::detectMarkers(undist, d, corners, ids, p);
    ...
    if (!ids.empty()){
        for (size_t i = 0; i < ids.size(); i++){
            if (ids[i] == 0){
                double msz = norm(corners[i][0] - corners[i][1]);
                aruco_dist = 0.10 * K.at<double>(0,0) / msz;
    ...
    if (roll_emergency && (in_climb || in_descend)){
        ...
        if (s0_phase == 0){
            ...
            if (isfinite(ob_x) && ob_x < 0.55 && ob_x > 0.1){
                if (aruco_detected && fabs(aruco_angle) > 0.06){
                    ...
                    s0_phase     = 1;
                    s0_align_cnt = 0;
                    cout << "[S0→ALIGN] aruco_angle=" << aruco_angle << endl;
                }else{
                    sc.StopMove();
        ...
        if(stair_cnt%10==0)
            cout<<"[S5] cnt="<<stair_cnt<<" err="<<err<<" vyaw="<<vyaw
                <<" vx="<<vx<<" d2d="<<d2d_turn<<" yaw="<<yaw<<endl;
